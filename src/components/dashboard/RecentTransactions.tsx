...
import { useCurrentUser } from "@/hooks/useCurrentUser"; // <--- Import User Hook
import { formatCurrency } from "@/lib/currency"; // <--- Import Formatter
import {
  CreditCard,
  DollarSign,
  Euro,
...
const IconWrapper = ({ type, currencyCode }: { type: string, currencyCode: string }) => {
  ...
  return (
    <div className={`w-12 h-12 rounded-full flex items-center justify-center ${bgColors[type]}`}>
      ...
      {/* Use dynamic currency icon for transfers */}
